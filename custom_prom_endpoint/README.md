...
- [Running Custom Prometheus Metricsndpoint](#running-custom-prometheus-metricsndpoint)
...
# TODO
* Put custome endpoint into a docker container
...
# Running Custom Prometheus Metricsndpoint
```
...
The metrics data are published as ordinary text; an example is given below. The upper porttion of the data contains standard, built-in metrics realated to the host system and Prometheus endopint itself, followed by our custom data, which begins at `buffer_level_bytes`.
```
...
scrape_configs:
  ...
  - job_name: 'custom prometheus'
    ...
    static_configs:
      # Note don'tuse "localhost" because it won't resolve inside the docker container
...
Prometheus has a rudimentary WebUI which lets you view stored metrics. Browse the WebUI at `locahost:9090`. Below is an example. You can start typing in the name of a metric into the "Expression" box and an autocomplete drop-down will help you choose a metric to display. You can view in tabular or graphical form.
![prometheus-webui-buffer-stats](images/prometheus-webui-buffer-stats.png)

# Grafana
## Install and Run Grafana using Docker
Ths will pull the image and start it on its default port (3000) as well as map the host port 3000 to the container port 3000:
```
...
Selecty a metric by typing into the **Metrics Browser** entry box, or using the pull-down widget and looking for a suitable candidate. In this case, we typed "buf" and the auto-complete presented a choce of `buffer_level_bytes` whcih we selected.
